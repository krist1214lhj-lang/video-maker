...
- `runAudioSubtitlePipeline()` — Audio+Subtitle 버튼
- `runAudioSubtitleOnlyFromStoryline()` — script/voice/subtitle만
- `handleFinalExportClick()` — Create Final Video (grant permission)
- `runSelectedCutsWithPostProduction({ runFinalExport: true })` — Generate All
- `invalidateFinalExportAfterAudioSubtitleOnly()`
...
- Hermes 진단: 예전에 `#generate-video-button`이 Final Export에 연결된 버그는 이미 분리됨.
- 터미널에서 Audio+Subtitle 직후 **GET /dashboard-status**만 반복되는 것은 정상 (4초 폴링).
- 사용자는 **한국어**로 응답 선호.
- git: main 브랜치, `templates/index.html` 등 다수 미커밋 변경 있음 (커밋은 사용자 요청 시만).

## Session End Message

> "내일하자" — 내일 이 파일을 읽고 Remaining Work부터 이어서 진행.
